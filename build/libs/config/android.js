  ...
  return _promise2.default.resolve().then(function () {
    ...
    return new _promise2.default(function (resolve, reject) {
      async.waterfall([function (callback) {
        ...
        var outString = data.replace(/sdk.dir.*/, 'sdk.dir=' + path.resolve(curPath, config.sdkdir));
        ...
        data = data.replace(/keyAlias.*/, 'keyAlias \'' + config.aliasname + '\'').replace(/applicationId.*/, 'applicationId \'' + config.packagename + '\'').replace(/keyPassword.*/, 'keyPassword \'' + config.password + '\'').replace(/storePassword.*/, 'storePassword \'' + config.storePassword + '\'').replace(/storeFile.*/, 'storeFile file(\'' + path.resolve(config.keystore) + '\')');
        ...
        data = data.replace(/package=".*"/, 'package="' + config.packagename + '"').replace(/android:versionCode=".*"/, 'android:versionCode="' + config.version.code + '"').replace(/android:versionName=".*"/, 'android:versionName="' + config.version.name + '"').replace(/android:name="weex_index"\sandroid:value=".*"/, 'android:name="weex_index" android:value="' + config.launch_path + '"');
